...
void displaySensorDetails(void)
{
  ...
  Mouse.screenSize(screenRes[0],screenRes[1]);
  Serial.println("------------------------------------");
...
//  vec[0] = diff[0]+diff[1]*1.0;
  ...
  int mov[2];
  for(int i = 0; i < 2; i++) {
    float offCenter = sin(vec[i])*distFromScreen;
    int pix = ((offCenter/screenDims[i])+0.5)*screenRes[i];
    mov[i] = max(0, min(screenRes[i], pix));
    Serial.println(mov[i]);
  }
//  if(mov[0] != 0 || mov[1] != 0)
